...
(def program
  ...
  [1102,34463338,34463338,63,1007,63,34463338,63,1005,63,53,1101,3,0,1000,109,988,209,12,9,1000,209,6,
   ...
   1005,1011,699,4,683,1001,64,1,64,1105,1,699,1002,64,2,64,109,29,2105,1,-7,1001,64,1,64,1105,
   1,717,4,705,1002,64,2,64,109,-19,21107,46,47,5,1005,1016,739,4,723,1001,64,1,64,1106,0,739,
...
(defn intcode
  "Extended version of the day 7 synchronous intcode interpreter,
     ...
     (let [load-from-pc    (fn [offset] (nth memory (+ pc offset) 0))
       ...
       (case opcode
         ...
         3  (recur (+ pc 2) rel-base  ; Read
                   (assoc-growing memory (resolve-address 0) (first inputs))
                   (rest inputs) outputs)
         ...
         5  (recur (if (zero? (resolve-operand 0))  ; Jump if true
                     (+ pc 3)
                     (resolve-operand 1))
                   rel-base memory inputs outputs)
         ...
         8  (recur (+ pc 4) rel-base  ; Equals
                   (assoc-growing memory (resolve-address 2)
                                  (if (= (resolve-operand 0) (resolve-operand 1)) 1 0))
                   inputs outputs)
         9  (recur (+ pc 2) (+ rel-base (resolve-operand 0)) memory inputs outputs)
         99 outputs)))))

(defn intcode-async
  "Asynchronous version of the extended intcode interpreter."
...
(defn intcode-async-wrapped
  "Synchronous wrapper for the interpreter for simple cases, accepting
  and returning vectors of input and output."
  ([input-data]
   (intcode program input-data))
  ([program-data input-data]
